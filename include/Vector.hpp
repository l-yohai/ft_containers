...
/*   Updated: 2020/12/01 00:23:16 by yohlee           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
public:
	explicit vector(const allocator_type & = allocator_type()) : _array(0), _size(0), _capacity(0) {}

	explicit vector(size_type n, const value_type& val = value_type(), const allocator_type & = allocator_type())
	: _array(new T[n]), _size(n), _capacity(n)
	{
	...
	vector(InputIterator first, InputIterator last, const allocator_type & = allocator_type(), typename std::enable_if<!std::is_integral<InputIterator>::value>::type * = 0)
	...
	vector(const vector& x) : _array(new T[x._capacity]), _size(x._size), _capacity(x._capacity)
	{
