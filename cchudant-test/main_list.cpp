...
/*   Updated: 2020/11/29 01:29:22 by yohlee           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
template <typename T>
static typename LIBRARY::list<T>::iterator get_n_fwd(LIBRARY::list<T> &lst, size_t index)
{
	size_t i = 0;
	for (typename LIBRARY::list<T>::iterator ite = lst.begin(); ite != lst.end(); ++ite)
	{
		if (i++ == index)
			return ite;
	}
...
template <typename T>
static typename LIBRARY::list<T>::reverse_iterator get_n_bwd(
	LIBRARY::list<T> &lst, size_t index)
{
	size_t i = lst.size() - 1;
	for (typename LIBRARY::list<T>::reverse_iterator ite = lst.rbegin();
		 ite != lst.rend(); ++ite)
		if (i-- == index)
			return ite;
	std::stringstream ss;
...
{
	LIBRARY::list<ConstrCounter> lst;
	assert(lst.size() == 0);
...
{
	{
		LIBRARY::list<ConstrCounter> lst(0, ConstrCounter(5));
		assert(lst.size() == 0);
	}
	{
		LIBRARY::list<ConstrCounter> lst(10, ConstrCounter(5));
		assert(lst.size() == 10);
		assert(get_n_fwd(lst, 5)->val == 5);
		assert(get_n_bwd(lst, 9)->val == 5);
	}
	{
		LIBRARY::list<ConstrCounter> lst(15, ConstrCounter(120));
		assert(lst.size() == 15);
		assert(get_n_fwd(lst, 0)->val == 120);
		assert(get_n_bwd(lst, 6)->val == 120);
	}
	{
		LIBRARY::list<ConstrCounter> lst(5, ConstrCounter(6));
		assert(lst.size() == 5);
		assert(get_n_fwd(lst, 2)->val == 6);
		assert(get_n_bwd(lst, 3)->val == 6);
	}
	{
		LIBRARY::list<ConstrCounter> lst(50, ConstrCounter(9000));
		assert(lst.size() == 50);
		assert(get_n_fwd(lst, 26)->val == 9000);
...
{
	{
		...
		LIBRARY::list<ConstrCounter> lst(array, array);
		assert(lst.size() == 0);
	}
	{
		ConstrCounter array[5] = {0, 1, 2, 3, 4};

		LIBRARY::list<ConstrCounter> lst(array, array + 5);
		assert(lst.size() == 5);
		assert(get_n_fwd(lst, 0)->val == 0);
		assert(get_n_bwd(lst, 1)->val == 1);
		assert(get_n_fwd(lst, 2)->val == 2);
	...
	}
	{
		...
		LIBRARY::list<ConstrCounter> lst(array, array + 2);
		assert(lst.size() == 2);
		assert(get_n_fwd(lst, 0)->val == 42);
		assert(get_n_fwd(lst, 1)->val == 43);
	}
...
static void test_assign_copy()
{
	...
	LIBRARY::list<ConstrCounter> lst2(lst);
	assert(lst2.size() == 3);
	...
	LIBRARY::list<ConstrCounter> lst3;
	assert(lst3.size() == 0);
...
static void test_iterator()
{
	...
	assert(lst.size() == 1);
	{
		LIBRARY::list<ConstrCounter>::iterator ite = lst.begin();
		++ite;
		assert(ite == lst.end());
	}
	...
	{
		LIBRARY::list<ConstrCounter>::iterator ite = lst.end();
		--ite;
		--ite;
		--ite;
	...
	assert(ite->val == 1);
	{
		LIBRARY::list<ConstrCounter>::iterator ite2 = lst.begin();
		++ite2;
		++ite2;
		assert(++ite == ite2);
	}
	...
	assert(ite == lst.end());
	{
		LIBRARY::list<ConstrCounter>::iterator ite2 = lst.end();
		--ite2;
		assert(--ite == ite2);
	}
	assert(++ite == lst.end());
...
static void test_reverse_iterator()
{
	...
	LIBRARY::list<ConstrCounter>::reverse_iterator ite =
		lst.rbegin();
	assert(ite->val == 2);
	++ite;
	assert(ite->val == 1);
...
static void test_insert_one()
{
...

	{
		LIBRARY::list<ConstrCounter>::iterator ite = lst.begin();
		++ite;
		LIBRARY::list<ConstrCounter>::iterator ret =
			lst.insert(ite, ConstrCounter(5));
		LIBRARY::list<ConstrCounter>::iterator exp = lst.begin();
		++exp;
		assert(ret == exp);
		assert(lst.size() == 2);
...
static void test_erase()
{
	...
	++ite;
	++ite;
	LIBRARY::list<ConstrCounter>::iterator ret = lst.erase(ite);
	LIBRARY::list<ConstrCounter>::iterator exp = lst.begin();
	++exp;
	++exp;
	assert(ret == exp);
...
static void test_erase_range()
{
	...
	LIBRARY::list<ConstrCounter>::iterator ite1 = lst.begin();
	++ite1;
	LIBRARY::list<ConstrCounter>::iterator ite2 = lst.end();
	--ite2;
	lst.erase(ite1, ite2);
...
static void test_swap()
{
	...
	LIBRARY::list<ConstrCounter> lst2(array2, array2 + 3);
...
static void test_cmp_eq()
{
	...
	LIBRARY::list<ConstrCounter> lst(array, array + 5);
	LIBRARY::list<ConstrCounter> lst2(lst);
	LIBRARY::list<ConstrCounter>::iterator it;
...
static void test_merge()
{
	...
	LIBRARY::list<ConstrCounter> lst2(array2, array2 + 12);
...
static void test_splice()
{
	...
	LIBRARY::list<ConstrCounter>::iterator pos = lst.begin();
	LIBRARY::list<ConstrCounter>::iterator ite = lst2.begin();
	LIBRARY::list<ConstrCounter>::iterator ite2 = lst2.begin();
	LIBRARY::advance(pos, 2);
	LIBRARY::advance(ite, 5);
	LIBRARY::advance(ite2, 7);
	...
	ite = lst2.begin();
	ite2 = lst2.begin();
	LIBRARY::advance(pos, 12);
	LIBRARY::advance(ite, 0);
	LIBRARY::advance(ite2, 0);
	...
	ite2 = lst2.begin();
	LIBRARY::advance(pos, 12);
	LIBRARY::advance(ite, 2);
	LIBRARY::advance(ite2, 8);
...
static void test_reverse()
{
	...
	LIBRARY::list<ConstrCounter> lst(array, array + 10);
	LIBRARY::list<ConstrCounter> lst2(reversed, reversed + 10);
...
static void test_sort()
{
	...
	LIBRARY::list<ConstrCounter> lst(array, array + 28);

	lst.sort();

	// for (LIBRARY::list<ConstrCounter>::iterator it = lst.begin(); it != lst.end(); ++it)
	// 	std::cout << it->val << ", ";
	// std::cout << std::endl;
	// for (LIBRARY::list<ConstrCounter>::reverse_iterator rit = lst.rbegin(); rit != lst.rend(); ++rit)
	// 	std::cout << rit->val << ", ";
	// std::cout << std::endl;
